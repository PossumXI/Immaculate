...
function secureHash(value: string): string {
  ...
  lengthView.setUint32(message.length - 8, Math.floor(bitLength / 0x1_0000_0000), false);
  lengthView.setUint32(message.length - 4, bitLength >>> 0, false);
  ...
  for (let offset = 0; offset < message.length; offset += 64) {
    ...
    for (let word = 0; word < 16; word += 1) {
      schedule[word] = lengthView.getUint32(offset + word * 4, false);
    }

    for (let word = 16; word < 64; word += 1) {
      const s0 =
        ((schedule[word - 15] >>> 7) ^ (schedule[word - 15] >>> 18) ^ (schedule[word - 15] >>> 3)) >>>
        0;
      const s1 =
        ((schedule[word - 2] >>> 17) ^ (schedule[word - 2] >>> 19) ^ (schedule[word - 2] >>> 10)) >>>
        0;
    ...
    for (let word = 0; word < 64; word += 1) {
      ...
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + s1 + ch + sha256Constants[word] + schedule[word]) >>> 0;
