...
ANCHORS = ("bridge", "http", "nonce", "timeout", "guard")
...
@criterion
def response_mentions_orchestration_facts(workspace: Path) -> float:
    response = _load_response(workspace)
    if response is None:
        return 0.0
    text = " ".join(
...
rk.response_mentions_orchestration_facts(weight=1.0)
