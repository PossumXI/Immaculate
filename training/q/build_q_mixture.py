...
def main() -> None:
    ...
    parser.add_argument("--supplemental", required=True, help="Supplemental JSON or JSONL dataset")
    parser.add_argument("--output", required=True, help="Combined output JSONL path")
    args = parser.parse_args()

    base_path = Path(args.base)
    supplemental_path = Path(args.supplemental)
    ...
    for record in supplemental_iter:
        ...
        if not text or text in seen:
            continue
        seen.add(text)
        tags = record.get("tags", record.get("labels", ["bridgebench", "q"]))
        rows.append(
            {
                "text": text,
                "source_id": record.get("source_id", "bridgebench-seed"),
                "relative_path": record.get("relative_path", record.get("id", "bridgebench-seed")),
