...
IMMACULATE_Q_TRAINING_BOOTSTRAP="${IMMACULATE_Q_TRAINING_BOOTSTRAP:-auto}"
...
if [[ "${IMMACULATE_Q_TRAINING_BOOTSTRAP}" == "always" || "${IMMACULATE_Q_TRAINING_BOOTSTRAP}" == "true" || ( "${IMMACULATE_Q_TRAINING_BOOTSTRAP}" == "auto" && "${IMMACULATE_Q_TRAINING_DRY_RUN}" != "true" ) ]]; then
  bash "${IMMACULATE_REPO_ROOT}/deploy/shared/scripts/install-q-training-stack.sh" "${IMMACULATE_Q_TRAINING_PYTHON}"
fi
