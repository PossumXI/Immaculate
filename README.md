...
The newest proof points are a real `Q` substrate benchmark where the gateway hands structured work back into Immaculate arbitration, a hard mixed-pressure reasoning lane where Immaculate follows Q without drift, a real `Q` API audit loop where live `/api/q/run` calls land as chained Arobi decision records instead of disappearing into logs, and a live Arobi audit-integrity lane that proves the request-to-decision-to-outcome chain stayed reviewable.
...
- live operator activity receipt: [docs/wiki/Live-Operator-Activity.md](docs/wiki/Live-Operator-Activity.md)
- live operator public export: [docs/wiki/Live-Operator-Public-Export.md](docs/wiki/Live-Operator-Public-Export.md)
...
- local Q runtime topology: [docs/wiki/Local-Q-Runtime-Topology.md](docs/wiki/Local-Q-Runtime-Topology.md)
...
- the Arobi live ledger receipt is now the truth surface for public-node health instead of a stale badge: [docs/wiki/Arobi-Live-Ledger-Receipt.md](docs/wiki/Arobi-Live-Ledger-Receipt.md) keeps the last verified supervised `396 -> 397` rerun from `2026-04-20T02:26:54Z` while also showing when the current public aura-genesis telemetry edge is synthesized/offline
...
- the supervised Discord/Q/OpenJaws loop now has its own machine-stamped activity page: [docs/wiki/Live-Operator-Activity.md](docs/wiki/Live-Operator-Activity.md) summarizes the live `Q` patrol lane, the current Discord transport receipts, the active roundtable session, and the latest bounded roundtable action receipts without duplicating the OpenJaws receipt schema or exposing worktree paths and private channel identifiers
...
- the tracked Q benchmark corpus currently carries `58` records, now including the new roundtable-runtime lane alongside the Arobi audit-integrity and roundtable-actionability lanes as positive benchmark evidence, and the hybrid training session plus Kaggle and Colab export lanes are restamped to the same `bench-v23` lineage
