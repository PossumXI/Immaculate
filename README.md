...
- the current HF Jobs lane is authenticated, hardware-visible, restaged against the active `bench-v16` lock, and launch-ready when you want to start the cloud run
- the current Cloudflare lane is still an inference/eval lane, not a claimed training backend: the profile and eval bundle are ready on `bench-v16`, while auth, worker config, and adapter export are still blocked
