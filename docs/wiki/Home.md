...
- the Arobi live ledger receipt is now the truth surface for public-node health instead of a stale green badge: [[Arobi-Live-Ledger-Receipt]] keeps the last verified supervised `396 -> 397` rerun from `2026-04-20T02:26:54Z` while also showing when the current public aura-genesis telemetry edge is synthesized/offline
- the new cross-project workflow receipt keeps the operational repo story honest across Immaculate, OpenJaws, and private Asgard_Arobi: [[Cross-Project-Workflow-Health]] samples the latest default-branch Actions runs, fails closed without authenticated access to private repos, and withholds private run links instead of pretending one green repo proves the whole stack is healthy
- the supervised public showcase now fail-closes by default: [[Supervised-Mission-Showcase]] publishes only the safe subset - roundtable runtime, the `5/5` public Terminal-Bench task, the supervised local Harbor tasks, and a sanitized public-edge ledger summary - while keeping the private mission lane closed until the shared mission gate is truly ready
...
- the Discord/OpenJaws activity loop now has a machine-stamped owner page of its own: [[Live-Operator-Activity]] summarizes the live `Q` patrol lane, Discord transport receipts, the active roundtable session, and the latest bounded roundtable action receipts without inventing a second receipt schema or exposing worktree paths and private channel IDs
- the public-safe bridge from that local operator loop into aura-genesis-compatible website copy now has its own stamped contract: [[Live-Operator-Public-Export]] mirrors the existing `fabric.showcase` field names, publishes only aggregate operator activity, and keeps the public line blocked until `ledger.public` is truly ready on this workstation
- the roundtable planning lane is now green both as a planner and as a live cold-start runtime proof: [[Roundtable-Actionability]] records `3` isolated repo-scoped actions across Immaculate, OpenJaws, and Asgard with explicit agent-branch-only authority plus a real bundle command, and [[Roundtable-Runtime]] now self-starts a dedicated local `Q` lane on `11435` while the everyday local `Q` lane stays on `11434`, proving those same lanes emit `3` governed execution bundles, `3` repo audit receipts, `3` bounded execution receipts, `3` task documents, `3` repo probes, and `3` authority-bound lanes with `0` failed assertions and a verified per-run decision-trace ledger
- the OpenJaws sidecar runtime remains green after a real coherence hardening pass: its live runtime-coherence report now checks the voice plane directly instead of treating a connected gateway alone as sufficient
- PoI is now a first-class Immaculate monitor instead of a website-only label: [[PoI-Agent-Assessment]] records the `poi-v1` scorecard, durable event schema, governed routes, and the automatic assessment loop that grades Q and future agents after executions and conversations
...
- the latest tracked W&B export remains current on `2026-04-20`, while the live mediation, substrate, gateway, public-task Harbor win, and audit-integrity benchmark above are the newer same-day repo-local wins
...
- [[Arobi-Live-Ledger-Receipt]] is the live truth surface for the public Arobi edge: it tells you whether aura-genesis is actually reading a live node right now or whether the public telemetry edge has fallen back to a synthesized/offline state
- [[Cross-Project-Workflow-Health]] is the machine-stamped GitHub Actions truth page for Immaculate, OpenJaws, and private Asgard_Arobi, with authenticated fallback for private repo reads and no public leakage of private run URLs
- [[Supervised-Mission-Showcase]] is the public-safe snippet pack for iorch and external handoff surfaces: it keeps the showcase fail-closed and keeps the private mission lane out of public copy until the shared mission gate is green
- [[Live-Operator-Activity]] is the machine-stamped local activity surface for the supervised Discord/Q/OpenJaws loop; it consumes canonical OpenJaws receipts, keeps the roundtable/action story discoverable for other agents, and does not prove public-ledger publication on its own
- [[Live-Operator-Public-Export]] is the public-safe aggregate export shaped for the aura-genesis `fabric.showcase` contract, so website/public-surface work can reuse one sanctioned field set instead of inventing parallel operator schemas
...
- [[Roundtable-Actionability]] is the live proof that Immaculate can turn a cross-project objective into isolated agent lanes with agent-branch-only write authority instead of storing a roundtable transcript without an execution plan
- [[Roundtable-Runtime]] is the live proof that the roundtable plan survives a real bounded Q run, mediated schedule, repo probe, and governed execution-bundle path instead of staying a planner-only claim
- [[PoI-Agent-Assessment]] records the first durable Proof of Intelligence monitor inside Immaculate: per-agent grades, scorecards, drift flags, evidence hashes, and governed assessment routes
...
- [[Local-Q-Runtime-Topology]] tracks the operator-facing split between the normal local Q lane on `11434`, the isolated roundtable lane on `11435`, and the matching OpenJaws dedicated local-Q override
