...
- the live Arobi public-node receipt is now green on the real `v3.3.1` lane: [[Arobi-Live-Ledger-Receipt]] records a fresh `control_fabric` entry at `2026-04-20T02:26:54Z`, public ledger entries moving `396 -> 397`, and a receipt match back to the latest supervised fabric-audit rerun
...
- [[Arobi-Live-Ledger-Receipt]] is the live proof that the public aura-genesis status and ledger pages are reading the real Arobi Network `v3.3.1` node and seeing fresh governed control_fabric records
